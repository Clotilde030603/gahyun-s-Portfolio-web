...
def board_view():
    ...
    status = 1
    select_query = """
    SELECT id, username, title, contents, real_filename, hash_filename FROM board WHERE id = ? and status = ?
    """
    cur.execute(select_query, (id, status))
    post = cur.fetchone() 

    # 첨부된 파일이 있는지 확인합니다.
    if post['hash_filename']:
        file_path = post['hash_filename']
    else:
        file_path = None

    return render_template('board_view.html', post=post, file_path=file_path)

@app.route('/download/<path:filename>', methods=['GET'])
def download_file(filename):
    # 파일이 저장된 디렉토리 경로를 가져옵니다.
    upload_directory = app.config['UPLOAD_FOLDER']

    # 요청된 파일의 전체 경로를 구성합니다.
    file_path = os.path.join(upload_directory, filename)

    # 요청된 파일이 존재하는지 확인하고 다운로드합니다.
